...
func newAutoScalingGroups(service *sgcloudWrapper) *autoScalingGroupCache {
...
func (m *autoScalingGroupCache) Register(asg *AutoScalingGroup) {
	m.cacheMutex.Lock()
...
func (m *autoScalingGroupCache) FindForInstance(instanceId string) (*AutoScalingGroup, error) {
	m.cacheMutex.Lock()
	defer m.cacheMutex.Unlock()
...
func (m *autoScalingGroupCache) regenerateCache() error {
	newCache := make(map[string]*AutoScalingGroup)

	for _, asg := range m.registeredAsgs {
		instances, err := m.service.getScalingInstancesByCluster(asg.config.manager.cloudConfig.ClusterID)
		if err != nil {
			return err
		}
